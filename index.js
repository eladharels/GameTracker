...
app.get('/api/system-status', authRequired, requirePermission('can_manage_users'), (req, res) => {
  ...
  statusService.checkAll({
    ...
    crackWatchCacheSize: crackwatch.cacheSize(),
  })
...
// The cache, its file and the refresh live in services/crackwatch.js (UP-16). What stays
// here is the process wiring: WHERE the file lives (CACHE_DIR, ephemeral), the boot load,
// the 04:00 cron and the first-boot refresh -- each of them server-process concerns.
crackwatch.init({ cacheDir: CACHE_DIR });
crackwatch.loadFromFile();

// Cron: refresh CrackWatch cache daily at 4:00 AM
scheduleWhenServer('0 4 * * *', () => {
  console.log('[CRON] Refreshing CrackWatch cache...');
  crackwatch.refresh().catch((err) => console.error('[CRON] CrackWatch refresh failed:', err));
});
...
if (isServerProcess) {
  setTimeout(() => {
    if (crackwatch.cacheSize() === 0) {
      console.log('[CrackWatch] Cache empty, running initial refresh in background...');
      crackwatch.refresh().catch((err) => console.error('[CrackWatch] Initial refresh failed:', err));
    }
  }, 30000);
}
...
app.post('/api/admin/refresh-crackwatch-cache', authRequired, requirePermission('can_manage_users'), async (req, res) => {
  try {
    await crackwatch.refresh();
    res.json({ success: true, message: 'CrackWatch cache refreshed', count: crackwatch.cacheSize() });
  } catch (err) {
...
// --- CrackRelease: single-game status. The scraper lives in services/crackwatch.js. ---
...
app.post('/api/admin/crackrelease-status', authRequired, requirePermission('can_manage_users'), crackCheckLimit, async (req, res) => {
  ...
  try {
    const result = await crackwatch.getCrackReleaseStatus(gameName);
    res.json(result);
  } catch (err) {
...
app.get('/api/crack-status/cache-info', authRequired, (req, res) => {
  res.json({ count: crackwatch.cacheSize(), sampleKeys: crackwatch.sampleKeys(5) });
});
